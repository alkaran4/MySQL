...
create table `comments` (
	id serial primary key, -- id комментария
	`body` varchar(1000) not null, -- тело комментария
	`user_id` BIGINT unsigned not null, -- кто написал комментарий
	`media_id` BIGINT unsigned not null, -- где оставлен комментарий
	`date_id` DATETIME DEFAULT NOW(), -- когда оставлен комментарий
	foreign key (`user_id`) references users(id) ON UPDATE CASCADE ON DELETE cascade,
	foreign key (`media_id`) references `photos`(id) ON UPDATE CASCADE ON DELETE cascade
);

drop table if exists `reposts`;
create table `reposts` (
	`count` serial primary key, -- счетчик просмотров 
	`user_id` BIGINT unsigned not null, -- кто посмотрел
	`media_id` BIGINT UNSIGNED NOT null, -- что посмотрел
	foreign key (`user_id`) references users(id) ON UPDATE CASCADE ON DELETE cascade,
	foreign key (`media_id`) references media(id) ON UPDATE CASCADE ON DELETE cascade
);
